...
fn get_short_import_symbol(buf: &[u8], f: &mut dyn FnMut(&[u8]) -> Result<()>) -> Result<bool> {
    ...
    let demangled_name = is_ec
        .then(|| get_arm64ec_demangled_function_name(unsafe { from_utf8_unchecked(name) }))
        .flatten()
        .map_or_else(
            || Cow::Borrowed(name),
...
fn get_img_rel_relocation(machine: MachineTypes) -> object::U16<object::LittleEndian> {
    u16!(match machine {
        ...
        MachineTypes::ARM64 | MachineTypes::ARM64EC => IMAGE_REL_ARM64_ADDR32NB,
        MachineTypes::I386 => IMAGE_REL_I386_DIR32NB,
        _ => panic!("unsupported machine"),
    })
}
...
impl<'a> ObjectFactory<'a> {
    fn new(s: &'a str, m: MachineTypes) -> Self {
        let import_as_path = PathBuf::from(s);
        let library = import_as_path.file_stem().unwrap().to_string_lossy();
    ...
    fn create_import_descriptor(&self) -> Result<NewArchiveMember> {
        ...
        pointer_to_symbol_table: u32!((size_of::<ImageFileHeader>() + (NUMBER_OF_SECTIONS * size_of::<ImageSectionHeader>()) +
            ...
            characteristics: u16!(if self.is_64_bit() { 0 } else { IMAGE_FILE_32BIT_MACHINE }),
    };
        buffer.write_all(unsafe { any_as_u8_slice(&header) })?;
        ...
        buffer.write_all(unsafe { any_as_u8_slice(&section_table) })?;
        ...
        buffer.write_all(unsafe { any_as_u8_slice(&import_descriptor) })?;
        ...
        buffer.write_all(unsafe { any_as_u8_slice(&relocation_table) })?;
        ...
        buffer.write_all(unsafe { any_as_u8_slice(&symbol_table) })?;
    ...
    fn create_short_import(
        ...
        buf.write_all(unsafe { any_as_u8_slice(&imp) })?;
    ...
    fn create_weak_external(
        ...
        let mut symbol_table: [_; NUMBER_OF_SYMBOLS] = [
            ...
            ImageSymbol {
                name: *b"\x40\x00\x00\x00\x00\x00\x00\x00",
                value: u32!(0),
                section_number: u16!(0),
                typ: u16!(0),
                storage_class: IMAGE_SYM_CLASS_EXTERNAL,
